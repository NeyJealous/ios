...
function envelope() {
  return {
    schemaVersion: '1.0.0', attestationId: 'attestation-fixture-0001',
    ...
    ownerApproval: { required: false, decision: 'NOT_REQUIRED', evidenceRef: null },
...
test('schema-valid PR-authored attestation remains insufficient evidence', () => {
  const result = evaluateTrustedAttestation({ attestation: envelope(), schema, expected, transport: 'PR_ARTIFACT' });
...
test('trusted transport still rejects stale head and model bindings', () => {
  ...
  const result = evaluateTrustedAttestation({ attestation: claim, schema, expected, transport: 'CODEX_RUNTIME_CHANNEL' });
  assert.equal(result.status, 'INSUFFICIENT_EVIDENCE');
...
test('owner-required claim cannot pass without attested approval', () => {
  const claim = envelope(); claim.ownerApproval = { required: true, decision: 'NOT_PROVIDED', evidenceRef: null };
  const result = evaluateTrustedAttestation({ attestation: claim, schema, expected, transport: 'GITHUB_OIDC_CHANNEL' });
  assert.equal(result.status, 'INSUFFICIENT_EVIDENCE');
  assert.ok(result.errors.includes('OWNER_APPROVAL_NOT_ATTESTED'));
});

test('fully bound envelope is structurally verifiable only through trusted transport', () => {
  const result = evaluateTrustedAttestation({ attestation: envelope(), schema, expected, transport: 'CODEX_RUNTIME_CHANNEL' });
