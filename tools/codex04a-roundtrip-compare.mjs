...
const target = resolve(tmpdir(), `ios-codex04a-roundtrip-${stamp}`);
...
const comparisons = localFiles.map((localName) => {
  ...
  try {
    ...
    return { localName, remoteName, equal: normalize(local) === normalize(remote), localSha256: sha256(local), remoteSha256: sha256(remote) };
  } catch (error) {
...
writeFileSync(resolve(root, 'audit', 'CODEX-04A-ROUNDTRIP.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
