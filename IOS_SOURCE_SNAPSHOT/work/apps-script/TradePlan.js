...
TI.TradePlan = {
  ...
  enrichRow: function(row, portfolio, ratings, constitution, reserve, regime, total) {
    ...
    var decision = rating ? String(rating.decision || "").trim() : "";
    ...
    var isBuy = !isReserveAction && (
      this.isBuyAction(action) ||
      actionLower.indexOf("куп") !== -1 ||
      actionLower.indexOf("сѓрї") !== -1
    );
    var reason = row.reason || "";

    if (rating) {
      row.rating = Number(rating.totalRating) || 0;
      reason = decision
        ? "Рейтинг: " + decision + "."
        : "Рейтинг: " + row.rating + ".";
    }

    if (decision === "Проверить продажу") {
      row.action = "Проверить продажу";
      row.status = "Проверить продажу";
      row.reason = "Рейтинг ниже нормы или отмечен фундаментальный риск.";
      row.comment = row.comment || "Продажа не выполняется автоматически. Нужна ручная проверка тезиса.";
      return row;
    }

    if (isBuy && decision === "Держать, но не увеличивать") {
      row.action = "Не покупать";
      row.status = "Ограничено рейтингом";
      row.reason = "Рейтинг 50-69: держать, но не увеличивать.";
      return row;
    }

    if (isBuy && ticker && this.exceedsPositionLimit(ticker, portfolio, row, total, constitution, ratings)) {
      row.action = "Не покупать";
      row.status = "Лимит позиции";
      row.reason = "Покупка превысит максимальную долю одной акции.";
      return row;
    }

    if (isBuy && ticker && this.exceedsSectorLimit(ticker, portfolio, row, total, constitution, ratings)) {
      row.action = "Не покупать";
      row.status = "Лимит сектора";
      row.reason = "Покупка превысит максимальную долю сектора.";
      return row;
    }

    if (isBuy && ticker && this.exceedsGoldLimit(ticker, portfolio, row, total, constitution, ratings)) {
      row.action = "Не покупать";
      row.status = "Лимит золота";
      row.reason = "Покупка превысит максимальную долю золота.";
      return row;
    }

    if (isBuy && ticker && this.exceedsHighRiskLimit(ticker, portfolio, row, total, constitution, ratings)) {
      row.action = "Не покупать";
      row.status = "Лимит риска";
      row.reason = "Покупка превысит максимальную долю высокорисковых идей.";
      return row;
    }

    if (isBuy && reserve.status === "Восстановить резерв") {
      row.status = row.status === "Готово" ? "Пониженный приоритет" : row.status;
      row.reason = "Сначала восстановить резерв. " + (reason || "");
      return row;
    }

    if (isBuy && this.isStockPurchase(row, ticker, portfolio, ratings) &&
        Number(regime.multiplier) > 1) {
      row.reason = "Режим рынка: " + regime.status +
        ", множитель покупки акций " + regime.multiplier + ". " + (reason || "");
      return row;
    }
