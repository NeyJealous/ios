...
TI.Diagnostics = {
  ...
  build: function() {
    ...
    checks = checks
      ...
      .concat(this.stabilizationChecks())
      .concat(this.inflationChecks())
      .concat(this.knowledgeChecks())
      .concat(this.multiAccountChecks())
      .concat(this.portfolioHealthChecks())
      .concat(this.portfolioIntelligenceChecks())
      .concat(this.assetIntelligenceChecks());
  ...
  accountChecks: function() {
    ...
    try {
      ...
      return [this.row(
        ...
        "Подключение",
        "Открытые счета",
        "Счетов в листе Счета: " + accounts.length + ".",
        accounts.length > 0
          ? ""
          : "Запустите Заполнить счета и стратегии."
      )];
    } catch (e) {
      return [this.row(
        this.STATUS.ERROR,
        ...
        "Открытые счета",
        e.message || String(e),
        "Проверьте лист Счета и настройки подключения."
      )];
    }
  },
    ...
    return [
      ...
      this.row(
        ...
        missingPrices.length === 0
          ? ""
          : "Запустите Обновить цены или проверьте служебные идентификаторы: " +
  ...
  stabilizationChecks: function() {
    var rows = TI.Data.sheetObjects(CORE.SHEETS.STABILIZATION);
    var errors = rows.filter(function(row) {
      return String(row.status || "") === TI.Diagnostics.STATUS.ERROR;
    }).length;
    var warnings = rows.filter(function(row) {
      return String(row.status || "") === TI.Diagnostics.STATUS.WARNING;
    }).length;

    return [this.row(
      rows.length > 0 && errors === 0 ? this.STATUS.OK : this.STATUS.WARNING,
      "Стабилизация",
      "Регрессионные проверки",
      "Проверок: " + rows.length +
        ", ошибок: " + errors +
        ", предупреждений: " + warnings + ".",
      rows.length > 0 ? "" : "Запустите стабилизационные проверки."
    )];
  },

  /**
   * Проверить слой фактов, признаков и оценок.
   * @return {Object[]}
   */
  knowledgeChecks: function() {
    var facts = TI.Data.sheetObjects(CORE.SHEETS.FACTS);
    var features = TI.Data.sheetObjects(CORE.SHEETS.FEATURES);
    var scores = TI.Data.sheetObjects(CORE.SHEETS.SCORES);

    return [
      this.row(
        facts.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Факты и оценки",
        "Факты",
        "Фактов найдено: " + facts.length + ".",
        facts.length > 0 ? "" : "Запустите Обновить факты и оценки."
      ),
      this.row(
        features.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Факты и оценки",
        "Признаки",
        "Признаков найдено: " + features.length + ".",
        features.length > 0 ? "" : "Запустите Обновить факты и оценки."
      ),
      this.row(
        scores.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Факты и оценки",
        "Оценки",
        "Оценок найдено: " + scores.length + ".",
        scores.length > 0 ? "" : "Запустите Обновить факты и оценки."
      )
    ];
  },

  /**
   * Проверить архитектуру нескольких счетов.
   * @return {Object[]}
   */
  multiAccountChecks: function() {
    var accounts = TI.MultiAccount.accounts();
    var strategies = TI.MultiAccount.strategies();
    var links = TI.MultiAccount.accountStrategies();

    return [
      this.row(
        accounts.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Счета",
        "Список счетов",
        "Счетов в управляющем листе: " + accounts.length + ".",
        accounts.length > 0 ? "" : "Запустите Заполнить счета и стратегии."
      ),
      this.row(
        strategies.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Счета",
        "Стратегии",
        "Стратегий найдено: " + strategies.length + ".",
        strategies.length > 0 ? "" : "Запустите Заполнить счета и стратегии."
      ),
      this.row(
        links.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Счета",
        "Связи счет-стратегия",
        "Связей найдено: " + links.length + ".",
        links.length > 0 ? "" : "Запустите Заполнить счета и стратегии."
      )
    ];
  },

  /**
   * Проверить здоровье портфеля.
   * @return {Object[]}
   */
  portfolioHealthChecks: function() {
    ...
    return [this.row(
      ...
      "Здоровье портфеля",
      "Расчет",
      "Строк здоровья портфеля: " + rows.length +
        ", предупреждений: " + warnings.length + ".",
      rows.length > 0 ? "" : "Запустите Оценить здоровье портфеля."
    )];
  },

  portfolioIntelligenceChecks: function() {
    var rows = TI.PortfolioIntelligence.read();
    ...
    return [
      this.row(
        ...
        "Интеллект портфеля",
        "Расчет",
        "Строк интеллекта портфеля: " + rows.length + ".",
        rows.length > 0 ? "" : "Запустите Рассчитать интеллект портфеля."
      ),
      this.row(
        bestAccount ? this.STATUS.OK : this.STATUS.WARNING,
        "Интеллект портфеля",
        "Лучший счет",
        bestAccount ? "Лучший счет: " + bestAccount.scopeName + "." : "Лучший счет не определен.",
        bestAccount ? "" : "Проверьте листы Счета и Здоровье портфеля."
      ),
      this.row(
        bestStrategy ? this.STATUS.OK : this.STATUS.WARNING,
        "Интеллект портфеля",
        "Лучшая стратегия",
        bestStrategy ? "Лучшая стратегия: " + bestStrategy.scopeName + "." : "Лучшая стратегия не определена.",
        bestStrategy ? "" : "Проверьте листы Стратегии и Стратегии счетов."
      )
    ];
  },

  assetIntelligenceChecks: function() {
    ...
    return [
      this.row(
        bonds.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Оценка активов",
        "Анализ облигаций",
        "Строк анализа облигаций: " + bonds.length + ".",
        bonds.length > 0 ? "" : "Запустите Рассчитать облигации."
      ),
      this.row(
        assets.length > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Оценка активов",
        "Единая оценка активов",
        "Строк оценки активов: " + assets.length + ".",
        assets.length > 0 ? "" : "Запустите Оценить активы."
      ),
      this.row(
        Number(coi.score || coi.value) > 0 ? this.STATUS.OK : this.STATUS.WARNING,
        "Индекс возможностей",
        "Расчет",
        "Текущий статус: " + (coi.status || "не рассчитан") + ".",
        Number(coi.score || coi.value) > 0 ? "" : "Запустите Рассчитать индекс возможностей."
      )
    ];
  },
