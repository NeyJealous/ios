...
TI.Portfolio = {
  ...
  build: function(options) {
    ...
    return this.applyPrices(rows, options);
  },
  ...
  /**
   * Получить названия счетов по ID из листа счетов.
   * @return {Object}
   */
   ...
   * @return {Object[]}
   */
  applyPrices: function(rows, options) {
    options = options || {};

    var prices = {};
    var rowsWithoutPrice = rows.filter(function(row) {
      return !Number(row.currentPrice);
    });
    var priceRows = options.skipPriceFetch === true
      ? []
      : (options.forcePrices === true ? rows : rowsWithoutPrice);

    if (priceRows.length > 0) {
      try {
        prices = TI.Prices.mapForPositions(priceRows);
      } catch (e) {
        Logger.log(e);
    ...
    rows.forEach(function(row) {
      var currentPrice =
        (options.forcePrices === true
          ? (prices[row.instrumentUid] || prices[row.figi] || Number(row.currentPrice))
          : (Number(row.currentPrice) || prices[row.instrumentUid] || prices[row.figi])) ||
        0;
  ...
  rebuild: function(options) {
    return this.write(this.build(options));
  },
